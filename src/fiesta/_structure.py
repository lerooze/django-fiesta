...
class StatusMessage(Registry):
    ...
    def update_status_message(self, status, message):
        self.status = status
        self.message_text = self.message_text or []
        language = (get_language(self._process_context.request.LANGUAGE_CODE))
        text_dataclass = default_class_loader('common', 'TextDataclass')
        with translation.override(language):
            txt = [text_dataclass(language, translation.gettext(message.text))]
        self.message_text.append(common_status_message_dataclass(text=txt,
                                                                 code=message.code))
...
@dataclass
class BaseRegistryInterface(BaseMessageDataclass):
...
class RegistryInterfaceSubmitStructureRequest(BaseRegistryInterface):
    ...
    def to_response(self):
        return RegistryInterfaceSubmitStructureResponseDataclass(
