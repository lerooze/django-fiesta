...
from dataclasses import fields, InitVar, is_dataclass, asdict, dataclass
...
from fiesta.core.coders import decode, encode
from fiesta.core.dataclasses import field, Empty
...
class Meta:
    ...
    attribute_fields: dict = None
    text_field: dict = None
    class_name: str = None


class BaseFiestaSerializer(type):

    @staticmethod
    def add_option(meta, _meta):
        if not meta: return _meta
        for option in ['namespace_key', 'app', 'model']:
            option_value = getattr(meta, option, None)
            if option_value:
                setattr(_meta, option, option_value)
        return _meta
    ...
    def __new__(metacls, name, bases, attrs, **kwargs):
        ...
        _meta = cls.add_option(meta, Meta())
        _meta.fields = cls.get_fields()
        _meta.ns_fields = cls.get_ns_fields(_meta)
        _meta.element_fields = cls.get_element_fields(_meta)
        _meta.attribute_fields = cls.get_atribute_fields(_meta)
        _meta.text_field = cls.get_text_field(_meta)
        _meta.class_name = cls.__name__
        _meta.model = cls.get_model(_meta)
        cls._meta = _meta
        return cls
    
    def get_model(cls, meta):
        if not apps: return None
        if not meta.model_name:
            model_name = meta.class_name.rsplit('Serializer')[0]
        else:
            model_name = meta.model_name
        try:
            return apps.get_model(meta.app, model_name)
        except LookupError:
            return
...
@dataclass
class FiestaSerializer(metaclass=BaseFiestaSerializer):
    ...
    def parse_element(self):
        meta = self._meta
        # Parsing element and assign values to fields

        # Parsing element attributes
        for attrib, attrib_value in self._element.attrib.items():
            qname = QName(attrib)
            try:
                f = meta.ns_fields[qname.namespace][True][
                    False][qname.localname]
            except KeyError:
                if getattr(self, 'extra_attribs', None):
                    self.extra_attribs.setdefault(
                        qname.namespace, {}
                    )[qname.localname] = attrib_value
            else:
                attrib_value = decode(attrib_value, f.type)
        ...
        if not len(self._element) and text:
            f = meta.text_field
            setattr(self, f.name, decode(text, f.type))


        # Parsing element with children
        for f, tag in meta.element_fields:
            if not issubclass(f.type, Iterable):
                ...
                if not is_dataclass(f.type):
                    ...
                    if f.type == Empty: setattr(self, f.name, True)
                        ...
                        setattr(self, f.name, decode(child_element.text,
                                                      f.type))
                else:
                    # Working on dataclass type children
                    setattr(self, f.name,
                            f.type(element=child_element))
            else:
                ...
                if is_dataclass(item_type):
                    ...
                    value = (decode(child.text, item_type) for child in
    ...
    def process(self, parent_obj=None, field_name=None, process_context=None):
        ...
        # This is the parent db object created from the parent process method
        # if not the root process 
        self._parent_obj = parent_obj

        # This is the field name of the field instance that is currently
        # processed
        ...
        # structure db object, the submission_results object, the submitted db
        # object and a stop variable that is set to true if need to stop the
        # current process early
        self._process_context = process_context 

        # This is the current db object created and is used to allow process
        ...
        # Validate the main obj before field instances are processed
        ...
        for f in self._meta.fields:
            ...
            if not isinstance(value, (Iterable, FiestaSerializer)): continue
            if isinstance(value, FiestaSerializer):
                child_obj = value.process(obj, f.name, self._process_context)
            else:
                item_type = f.type.__args__[0]
                if issubclass(item_type, FiestaSerializer): 
                    child_obj= ( 
                        item.process(obj, f.name, self._process_context)
                        for item in value
                    ) 
                else:
                    child_obj = value
    ...
    @staticmethod
    def _extend(qname, values, item_type, unroll):
        if not issubclass(item_type, FiestaSerializer):
            for value in values:
                element = Element(qname.text)
                element.text = encode(value, item_type)
                yield element
        else:
            for value in values:
    ...
    def unroll(self):
        for f in fields(self):
            ...
            if isinstance(value, Iterable) and type(value) != str:
                values = []
                for v in value:
                    if isinstance(v, FiestaSerializer): values.append(v.unroll())
                    else: values.append(v)
                setattr(self, f.name, values)
            elif isinstance(value, FiestaSerializer):
    ...
    def to_element(self, tag=None, unroll=False):
        ...
        attrib = dict(
            (qname.text, encode(getval(f), f.type))
             for f, qname in meta.attribute_fields
        ...
        if extra_attribs:
            ...
            attrib.extend(extra)
        if not tag:
            tag = QName(constants.NAMESPACE[meta.namespace_key], meta.class_name
                       ).text
            nsmap = constants.NAMESPACE_CLEAN 
        else:
            nsmap = None
        element = Element(tag, attrib=attrib, nsmap=nsmap)
        if meta.text_field:
            f = meta.text_field
            element.text = encode(getval(f), f.type)
        for f, qname in meta.element_fields:
            value = getval(f)
            ...
            if isinstance(value, Empty):
                ...
                element.extend(self._extend(qname, value, item_type, unroll))
            else:
                new_elem = Element(qname.text)
                new_elem.text = encode(value, f.type)
                element.append(new_elem)
        return element

    def append_error(self, status, detail=None):
        if detail: 
            description = f'{status.description}: {detail}'
        else:
...
class Sender(Party):
    ...
    def preprocess_isvalid(self):
        ...
        if request.user.contact.organisation != sender:
            self.append_error(
                status.FIESTA_1003_USER_NOT_IN_ORGANISATION,
                f'user is {self.request.user}, organisation is {sender}'
            )
        else:
            self._obj = sender
...
class OrganisationSchemes(BaseStructure):
    _model = apps.get_model('base', 'OrganisationScheme')
...
class Structures(BaseStructure):
    ...
    def retrieve(self, registration):
        if registration.resource == 'organisationscheme':
            self.organisation_schemes = OrganisationSchemes.retrieve(registration)
        elif registration.resource == 'codelist':
            self.codelists = Codelists.retrieve(registration)
        elif registration.resource == 'conceptscheme':
            self.codelists = Concepts.retrieve(registration)
        ...
        if not self.structures:
            ...
            if not isinstance(message, Structure):
                ...
                self.append_error(
                    ...
                    f'location of pulled message {loc}'
                )
                self._process_context.stop = True
